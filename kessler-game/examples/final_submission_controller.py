...
class AEL_CompetitionController(KesslerController):
    ...
    def __init__(self, chromosome):
        ...
        collision_time = ctrl.Antecedent(np.arange(0, 30, 0.1), 'collision_time')
        ship_turn = ctrl.Consequent(np.arange(-180, 180, 1), 'ship_turn')  # Degrees due to Kessler
        ship_fire = ctrl.Consequent(np.arange(-1, 1, 0.1), 'ship_fire')
        thrust = ctrl.Consequent(np.arange(0.0, 480.0, 0.1), 'thrust') # Anything beyond 120 is gets tough to control
        ...
        means = [mean*30 for mean in chromosome[0].value[0]]
        std_devs = [std_dev*30 for std_dev in chromosome[0].value[1]]
        collision_time['S'] = fuzz.gaussmf(collision_time.universe, means[0], std_devs[0])
        collision_time['M'] = fuzz.gaussmf(collision_time.universe, means[1], std_devs[1])
        collision_time['L'] = fuzz.gaussmf(collision_time.universe, means[2], std_devs[2])
        ...
        # thrust['PS'] = fuzz.trimf(thrust.universe, [0.0, 0.0, 240.0])
        # thrust['PM'] = fuzz.trimf(thrust.universe, [120.0, 240.0, 360.0])
        # thrust['PL'] = fuzz.trimf(thrust.universe, [240.0, 480.0, 480.0])

        means = [mean * 480 for mean in chromosome[1].value[0]]
        std_devs = [std_dev * 480 for std_dev in chromosome[1].value[1]]
        thrust['PS'] = fuzz.gaussmf(thrust.universe, means[0], std_devs[0])
        thrust['PM'] = fuzz.gaussmf(thrust.universe, means[1], std_devs[1])
        thrust['PL'] = fuzz.gaussmf(thrust.universe, means[2], std_devs[2])
        ...
        # Declare each fuzzy rule
        mc_rule1 = ctrl.Rule(collision_time['L'] & theta_delta['NL'], (ship_turn['NL'], thrust['PS'], ship_mine['N']))
        mc_rule2 = ctrl.Rule(collision_time['L'] & theta_delta['NM'], (ship_turn['NM'], thrust['PS'], ship_mine['N']))
        mc_rule3 = ctrl.Rule(collision_time['L'] & theta_delta['NS'], (ship_turn['NS'], thrust['PM'], ship_mine['N']))
        mc_rule5 = ctrl.Rule(collision_time['L'] & theta_delta['PS'], (ship_turn['PS'], thrust['PM'], ship_mine['N']))
        mc_rule6 = ctrl.Rule(collision_time['L'] & theta_delta['PM'], (ship_turn['PM'], thrust['PS'], ship_mine['N']))
        mc_rule7 = ctrl.Rule(collision_time['L'] & theta_delta['PL'], (ship_turn['PL'], thrust['PS'], ship_mine['N']))
        mc_rule8 = ctrl.Rule(collision_time['M'] & theta_delta['NL'], (ship_turn['NL'], thrust['PM'], ship_mine['N']))
        mc_rule9 = ctrl.Rule(collision_time['M'] & theta_delta['NM'], (ship_turn['NM'], thrust['PM'], ship_mine['N']))
        mc_rule10 = ctrl.Rule(collision_time['M'] & theta_delta['NS'], (ship_turn['NS'], thrust['PM'], ship_mine['N']))
        mc_rule12 = ctrl.Rule(collision_time['M'] & theta_delta['PS'], (ship_turn['PS'], thrust['PM'], ship_mine['N']))
        mc_rule13 = ctrl.Rule(collision_time['M'] & theta_delta['PM'], (ship_turn['PM'], thrust['PM'], ship_mine['N']))
        mc_rule14 = ctrl.Rule(collision_time['M'] & theta_delta['PL'], (ship_turn['PL'], thrust['PS'], ship_mine['N']))
        mc_rule15 = ctrl.Rule(collision_time['S'] & theta_delta['NL'], (ship_turn['NL'], thrust['PM'], ship_mine['N']))
        mc_rule16 = ctrl.Rule(collision_time['S'] & theta_delta['NM'], (ship_turn['NM'], thrust['PM'], ship_mine['N']))
        mc_rule17 = ctrl.Rule(collision_time['S'] & theta_delta['NS'], (ship_turn['NS'], thrust['PL'], ship_mine['Y']))
        mc_rule19 = ctrl.Rule(collision_time['S'] & theta_delta['PS'], (ship_turn['PS'], thrust['PL'], ship_mine['Y']))
        mc_rule20 = ctrl.Rule(collision_time['S'] & theta_delta['PM'], (ship_turn['PM'], thrust['PM'], ship_mine['N']))
        mc_rule21 = ctrl.Rule(collision_time['S'] & theta_delta['PL'], (ship_turn['PL'], thrust['PM'], ship_mine['N']))
    ...
    def actions(self, ship_state: Dict, game_state: Dict) -> Tuple[float, float, bool, bool]:
        ...
        if danger_level/2 > ship_state["fire_rate"]:
            ...
            num_directions = 30 # This could totally screw up performance - might need to reduce
            collision_threshold = 100.0  # idk
            best_direction = None
            best_length = -1.0
            ...
            # print(f"Turning {turn_rate} degrees and thrusting {thrust} units")
            fire = False
            drop_mine = moving.output['ship_mine'] > 0
