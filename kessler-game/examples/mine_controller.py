...
class MineController(KesslerController):
    ...
    def __init__(self):
        ...
        mine_danger = ctrl.Antecedent(np.arange(0, 100, 1), 'mine_danger')
        ...
        mine_danger['L'] = fuzz.zmf(mine_danger.universe, 75, 100)
        mine_danger['M'] = fuzz.trimf(mine_danger.universe, [25, 50, 75])
        mine_danger['S'] = fuzz.smf(mine_danger.universe, 0, 25)
        ...
        ship_move['L'] = fuzz.trimf(ship_move.universe, [-210, -185, -135])
        ship_move['M'] = fuzz.trimf(ship_move.universe, [-160, -135, -110])
        ship_move['S'] = fuzz.trimf(ship_move.universe, [-110, -25, 0])

        ship_mine['Y'] = fuzz.trimf(ship_fire.universe, [-1, -1, 0.0])
        ship_mine['Y'] = fuzz.trimf(ship_fire.universe, [0.0, 1, 1])

        # Declare each fuzzy rule
        rule1 = ctrl.Rule(bullet_time['L'] & theta_delta['NL'],
                          (ship_turn['NL'], ship_fire['N'], ship_move['S'], ship_mine['Y']))
        rule2 = ctrl.Rule(bullet_time['L'] & theta_delta['NM'],
                          (ship_turn['NM'], ship_fire['N'], ship_move['M'], ship_mine['Y']))
        rule3 = ctrl.Rule(bullet_time['L'] & theta_delta['NS'],
                          (ship_turn['NS'], ship_fire['Y'], ship_move['L'], ship_mine['Y']))
        ...
        rule5 = ctrl.Rule(bullet_time['L'] & theta_delta['PS'],
                          (ship_turn['PS'], ship_fire['Y'], ship_move['L'], ship_mine['Y']))
        rule6 = ctrl.Rule(bullet_time['L'] & theta_delta['PM'],
                          (ship_turn['PM'], ship_fire['N'], ship_move['M'], ship_mine['Y']))
        rule7 = ctrl.Rule(bullet_time['L'] & theta_delta['PL'],
                          (ship_turn['PL'], ship_fire['N'], ship_move['S'], ship_mine['Y']))
        rule8 = ctrl.Rule(bullet_time['M'] & theta_delta['NL'],
                          (ship_turn['NL'], ship_fire['N'], ship_move['M'], ship_mine['Y']))
        rule9 = ctrl.Rule(bullet_time['M'] & theta_delta['NM'],
                          (ship_turn['NM'], ship_fire['N'], ship_move['L'], ship_mine['Y']))
        ...
        rule13 = ctrl.Rule(bullet_time['M'] & theta_delta['PM'],
                           (ship_turn['PM'], ship_fire['N'], ship_move['L'], ship_mine['Y']))
        rule14 = ctrl.Rule(bullet_time['M'] & theta_delta['PL'],
                           (ship_turn['PL'], ship_fire['N'], ship_move['M'], ship_mine['Y']))
        ...
        rule16 = ctrl.Rule(bullet_time['S'] & theta_delta['NM'],
                           (ship_turn['NM'], ship_fire['Y'], ship_move['L'], ship_mine['Y']))
        ...
        rule20 = ctrl.Rule(bullet_time['S'] & theta_delta['PM'],
                           (ship_turn['PM'], ship_fire['Y'], ship_move['L'], ship_mine['Y']))
    ...
    def actions(self, ship_state: Dict, game_state: Dict) -> Tuple[float, float, bool]:
        ...
        intrcpt_x = closest_asteroid["aster"]["position"][0] + closest_asteroid["aster"]["velocity"][0] * (
                    bullet_t + 1 / 30)
        intrcpt_y = closest_asteroid["aster"]["position"][1] + closest_asteroid["aster"]["velocity"][1] * (
                    bullet_t + 1 / 30)
        ...
        if closest_mine is not None and abs(closest_mine["dist"]) < 200.0:
            ...
            if shooting_theta > 90:
                shooting_theta -= 180
            elif shooting_theta < -90:
                shooting_theta += 180
        else:
            mine_threat = 0
        ...
        # shooting.input['mine_danger'] = mine_threat
        ...
        if mine_threat < 100:
            ...
            thrust = 300 if abs(shooting_theta) > math.pi / 2 else -300
        ...
        if shooting.output['ship_mine'] >= 0:
            drop_mine = True
        else:
            drop_mine = False

        # DEBUG
        print(mine_threat, thrust, bullet_t, shooting_theta, turn_rate, fire)

        return thrust, turn_rate, fire, drop_mine

    @property
    def name(self) -> str:
        return "AEL Controller"